...
function getStatusBadge(status) {
  ...
  if (["OW", "OB", "T"].includes(status))
    return `<span class="badge-status badge-warning">${status}</span>`;
  if (["UW", "SUW", "ST", "SST", "MW", "SW"].includes(status))
    return `<span class="badge-status badge-danger">${status}</span>`;
...
function filterDataList(list, ageRange, status, vitStatus) {
  ...
  if (ageRange && ageRange !== "all") {
    ...
    else if (ageRange === "12-23")
      filtered = filtered.filter((k) => k.age >= 12 && k.age <= 23);
    else if (ageRange === "24-59")
      filtered = filtered.filter((k) => k.age >= 24);
  }
  if (status && status !== "all")
    filtered = filtered.filter((k) => k.overallStatus === status);
  if (vitStatus && vitStatus !== "all")
    filtered = filtered.filter(
      (k) => k.vitaminTaken === (vitStatus === "Complete"),
    );
  return filtered;
}
...
function initFilters() {
  ...
  balayanBrgys.forEach(
    (b) => (optionsTrendsRecords += `<option value="${b}">Brgy. ${b}</option>`),
  );
  selectsTrendsRecords.forEach((select) => {
    if (select) select.innerHTML = optionsTrendsRecords;
  });
  ...
  if (reportBrgyFilter) {
    ...
    balayanBrgys.forEach(
      (b) => (optionsReports += `<option value="${b}">Brgy. ${b}</option>`),
    );
...
function openInfoPanel(
  ...
  if (badCases === 0) {
    ...
    aiAction =
      "Continue sustaining current BNS monitoring and standard vitamin distributions.";
    ...
    aiAction =
      mal > obese
        ? `Schedule targeted nutritional counseling focusing on the ${mal} cases of malnutrition.`
        : `Initiate awareness seminars focusing on balanced diets to address the ${obese} cases of obesity.`;
  } else {
    aiAnalysis = `Brgy. ${brgy} maintains a stable health baseline. The vast majority of children (${normal}) are healthy.`;
    aiAction =
      "Maintain standard tracking protocols and routine monthly checkup follow-ups.";
  }
  ...
  content.innerHTML = `
        ...
        <div class="ai-suggestion-box">
            <div class="ai-header">
                <img src="Gemini.png" alt="Gemini Logo" class="gemini-logo"> 
...
function closeInfoPanel() {
  if (document.getElementById("brgy-info-panel")) {
    document.getElementById("brgy-info-panel").classList.remove("open");
  }
}
...
function updateTrends() {
  ...
  let baseKids =
    brgy === "all" ? Object.values(masterData).flat() : masterData[brgy];
  let filteredKids = baseKids;

  if (ageRange !== "all") {
    if (ageRange === "0-11")
      filteredKids = filteredKids.filter((k) => k.age <= 11);
    else if (ageRange === "12-23")
      filteredKids = filteredKids.filter((k) => k.age >= 12 && k.age <= 23);
    else if (ageRange === "24-59")
      filteredKids = filteredKids.filter((k) => k.age >= 24);
  }
  ...
  if (selectedMetric === "WFLH") {
    addDataset(
      "Normal (N)",
      "#2e7d32",
      filteredKids.filter((k) => k.wflh === "N").length,
    );
    addDataset(
      "Overweight (OW)",
      "#fbc02d",
      filteredKids.filter((k) => k.wflh === "OW").length,
    );
    addDataset(
      "Obese (OB)",
      "#e65100",
      filteredKids.filter((k) => k.wflh === "OB").length,
    );
    addDataset(
      "Mod. Wasted (MW)",
      "#f44336",
      filteredKids.filter((k) => k.wflh === "MW").length,
    );
    addDataset(
      "Sev. Wasted (SW)",
      "#b71c1c",
      filteredKids.filter((k) => k.wflh === "SW").length,
    );
  } else if (selectedMetric === "HFA") {
    addDataset(
      "Normal (N)",
      "#2e7d32",
      filteredKids.filter((k) => k.hfa === "N").length,
    );
    addDataset(
      "Tall (T)",
      "#1976d2",
      filteredKids.filter((k) => k.hfa === "T").length,
    );
    addDataset(
      "Stunted (ST)",
      "#f44336",
      filteredKids.filter((k) => k.hfa === "ST").length,
    );
    addDataset(
      "Sev. Stunted (SST)",
      "#b71c1c",
      filteredKids.filter((k) => k.hfa === "SST").length,
    );
  } else if (selectedMetric === "WFA") {
    addDataset(
      "Normal (N)",
      "#2e7d32",
      filteredKids.filter((k) => k.wfa === "N").length,
    );
    addDataset(
      "Overweight (OW)",
      "#fbc02d",
      filteredKids.filter((k) => k.wfa === "OW").length,
    );
    addDataset(
      "Underweight (UW)",
      "#f44336",
      filteredKids.filter((k) => k.wfa === "UW").length,
    );
    addDataset(
      "Sev. Underweight (SUW)",
      "#b71c1c",
      filteredKids.filter((k) => k.wfa === "SUW").length,
    );
  }
  ...
  const ctxBar = document
    .getElementById("brgyComparisonChart")
    .getContext("2d");
  ...
  balayanBrgys.forEach((b) => {
    ...
    if (ageRange !== "all") {
      if (ageRange === "0-11") list = list.filter((k) => k.age <= 11);
        ...
        list = list.filter((k) => k.age >= 12 && k.age <= 23);
  ...
  comparisonChart = new Chart(ctxBar, {
    ...
    data: {
      ...
      datasets: [
        {
          label: `Total Issues (${selectedMetric})`,
          data: barData,
...
function updateRecords() {
  ...
  if (document.getElementById("statTotalRecords"))
    document.getElementById("statTotalRecords").innerText = totalReg;
  if (document.getElementById("statMissedRecords"))
  ...
  document.getElementById("records-table-body").innerHTML = filteredList
    .slice(0, 100)
    .map((k) => {
      let vitaminDisplay = k.vitaminTaken
        ? `<span class="badge complete" style="font-size:10px; padding: 4px 8px;"><i class="fas fa-check"></i> Complete</span>`
        : getVitaminsByAge(k.age)
            .map(
              (v) =>
                `<div style="color: #c62828; font-size: 10px; margin-bottom: 2px;"><i class="fas fa-times"></i> ${v}</div>`,
            )
            .join("");
...
function updateReportsView() {
  ...
  if (brgyFilter === "all") {
    ...
    ["PUROK 1", "PUROK 2", "PUROK 3", "PUROK 4", "PUROK 5"].forEach((purok) => {
      reportListHTML += generateSummaryRow(
        `${brgyFilter} - ${purok}`,
        kidsInBrgy.filter((k) => k.sitio === purok),
      );
    });
  ...
  if (totals.kids.length > 0)
    reportListHTML += generateSummaryRow("GRAND TOTAL", totals.kids).replace(
      "<tr>",
      '<tr style="background-color:#f0f4f1;">',
    );

  tbody.innerHTML =
    reportListHTML || `<tr><td colspan="15">No records.</td></tr>`;
}
