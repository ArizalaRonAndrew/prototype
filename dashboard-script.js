...
function switchView(viewName) {
  if (document.getElementById("trends-view")) {
    document.getElementById("trends-view").style.display =
      viewName === "trends" ? "block" : "none";
    document.getElementById("records-view").style.display =
      viewName === "records" ? "block" : "none";

    if (document.getElementById("reports-view")) {
      document.getElementById("reports-view").style.display =
        viewName === "reports" ? "block" : "none";
    }
...
function initFilters() {
  ...
  for (let i = 1; i <= brgyCount; i++)
    options += `<option value="Brgy ${i}">Barangay ${i}</option>`;
...
function updateTrends() {
  ...
  if (ageRange !== "all") {
    if (ageRange === "0-11") kids = kids.filter((k) => k.age <= 11);
      ...
      kids = kids.filter((k) => k.age >= 12 && k.age <= 23);
    else kids = kids.filter((k) => k.age >= 24);
  }

  document.getElementById("statNormal").innerText = kids.filter(
    (k) => k.status === "Normal",
  ).length;
  document.getElementById("statMal").innerText = kids.filter(
    (k) => k.status === "Underweight" || k.status === "Stunted",
  ).length;
  document.getElementById("statObese").innerText = kids.filter(
    (k) => k.status === "Obese",
  ).length;
  ...
  const createMonthlyData = (baseCount) =>
    months.map(() => Math.floor(Math.random() * (baseCount / 4)) + 2);
  const datasets = [];

  if (selectedStatus === "all" || selectedStatus === "Normal") {
    datasets.push({
      label: "Normal",
      data: createMonthlyData(kids.filter((k) => k.status === "Normal").length),
      borderColor: "#2e7d32",
  ...
  if (
    selectedStatus === "all" ||
    selectedStatus === "Underweight" ||
    selectedStatus === "Stunted"
  ) {
    datasets.push({
      label: "Malnourished",
      data: createMonthlyData(
        kids.filter((k) => k.status === "Underweight" || k.status === "Stunted")
          .length,
      ),
      borderColor: "#d32f2f",
      backgroundColor: "transparent",
  ...
  if (selectedStatus === "all" || selectedStatus === "Obese") {
    datasets.push({
      label: "Obese",
      data: createMonthlyData(kids.filter((k) => k.status === "Obese").length),
      borderColor: "#fbc02d",
      backgroundColor: "transparent",
      tension: 0.4,
    });
...
function updateRecords() {
  ...
  if (vitStatus !== "all")
    list = list.filter((k) => k.vitaminTaken === (vitStatus === "Complete"));
